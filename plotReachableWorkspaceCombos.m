function plotReachableWorkspaceCombos(linkLengths)
    ...
    fprintf("B done");
    scatterHandles.C = plotComboC(linkLengths, jointLimitsRad, CcoarseResRad, CfineResRad, ax);
    fprintf("C done");
    ...
    uicontrol('Style', 'checkbox', 'String', 'Show Combo C', ...
        'Position', [280, 20, 120, 20], 'Value', 1, ...
        'Callback', @(src,~) toggleVisibility(scatterHandles.C, src.Value));

    for i = 1:length(scatterHandles.C)
        uicontrol('Style', 'checkbox', 'String', sprintf('Sphere %d', i), ...
        'Position', [20, 50 + 25*i, 120, 20], 'Value', 1, ...
        'Callback', @(src,~) toggleVisibility(scatterHandles.C(i), src.Value));
    end
...
function h = plotComboC(linkLengths, jointLimitsRad, coarseResRad, fineResRad, ax)
    % Define a colormap
    colormapList = lines(64); % Up to 64 distinguishable colors
    colorIndex = 1;
    h = gobjects(0); % Preallocate graphic object array
    ...
    for theta2 = jointLimitsRad(2,1):coarseResRad:jointLimitsRad(2,2)
        for theta3 = jointLimitsRad(3,1):coarseResRad:jointLimitsRad(3,2)
            ...
            % Wrap around if colorIndex exceeds colormap size
            thisColor = colormapList(mod(colorIndex-1, size(colormapList,1)) + 1, :);
            colorIndex = colorIndex + 1;

            h(end+1) = scatter3(ax, positions(:,1), positions(:,2), positions(:,3), ...
                                10, thisColor, 'filled', ...
                                'DisplayName', sprintf('C: θ₂=%.1f, θ₃=%.1f', theta2, theta3));
        end
    end
end